...
class UpdateDataForm(FlaskForm):
    ...
    racial_identity = StringField(label='Racial Identities (comma seperated)', validators=[Optional(strip_whitespace=True)])
    edit_comment = StringField(label='Edit Comment', validators=[Optional(strip_whitespace=True)])
